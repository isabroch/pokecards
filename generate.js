...
function formOptions() {
  ...
  populateForm(formBalls[i], formBalls[i], formBallSelect);
  populateForm(formItems[i][1], formItems[i][2], formItemSelect);
  populateForm(formGenders[i], formGenders[i], formGenderSelect);
...
function populateForm(value, text, form) {
  for (var i = 0; i < array.length; i++) {
    form[form.length] = new Option(value, text);
  }
}
